...
function traverseFunDef(c, s) {
    ...
    // console.log("Before pop to def: ", c.type.name);
    c.parent(); // Pop to FunctionDefinition
    return { name: name, parameters: parameters, ret: ret, inits: inits, body: body };
}
...
function traverseClass(c, s) {
    ...
    if (!methods.find(function (method) { return method.name === "__init__"; })) {
        methods.push({ name: "__init__", parameters: [{ name: "self", type: utils_1.CLASS(className) }], ret: utils_1.NONE, inits: [], body: [] });
    }
    return {
        name: className,
        fields: fields,
...
function parse(source) {
    ...
    var str = treeprinter_1.stringifyTree(t.cursor(), source, 0);
