...
export function lowerProgram(
  ...
  var firstBlock: IR.BasicBlock<Type> = {
    a: p.a,
    label: generateName("$startProg"),
    stmts: [],
  };
...
function lowerFunDef(f: AST.FunDef<Type>, env: GlobalEnv): IR.FunDef<Type> {
  ...
  return {
    ...f,
    inits: [...bodyinits, ...lowerVarInits(f.inits, env)],
    body: blocks,
  };
}

function lowerVarInits(
  inits: Array<AST.VarInit<Type>>,
  env: GlobalEnv
): Array<IR.VarInit<Type>> {
  return inits.map((i) => lowerVarInit(i, env));
}

function lowerVarInit(
  init: AST.VarInit<Type>,
  env: GlobalEnv
): IR.VarInit<Type> {
  return {
    ...init,
...
function lowerClasses(
  classes: Array<AST.Class<Type>>,
  env: GlobalEnv
): Array<IR.Class<Type>> {
...
function flattenStmt(
  ...
  switch (s.tag) {
    case "assign":
      ...
      blocks[blocks.length - 1].stmts.push(...valstmts, {
        a: s.a,
        tag: "assign",
        name: s.name,
        value: vale,
    ...
    case "field-assign": {
      ...
      const offset: IR.Value<Type> = {
        tag: "wasmint",
        value: classdata.get(s.field)[0],
      };
    ...
    case "while":
      ...
      var [cinits, cstmts, cexpr] = flattenExprToVal(s.cond, env);
      pushStmtsToLastBlock(blocks, ...cstmts, {
        tag: "ifjmp",
        cond: cexpr,
        thn: whilebodyLbl,
        els: whileEndLbl,
...
function flattenExprToExpr(
  e: AST.Expr<Type>,
  env: GlobalEnv
): [Array<IR.VarInit<Type>>, Array<IR.Stmt<Type>>, IR.Expr<Type>] {
  switch (e.tag) {
    case "uniop":
    ...
    case "builtin1":
      ...
      return [
        inits,
        stmts,
        { tag: "builtin1", a: e.a, name: e.name, arg: val },
      ];
    case "builtin2":
      var [linits, lstmts, lval] = flattenExprToVal(e.left, env);
      var [rinits, rstmts, rval] = flattenExprToVal(e.right, env);
    ...
    case "method-call": {
      ...
      const checkObj: IR.Stmt<Type> = {
        tag: "expr",
        expr: { tag: "call", name: `assert_not_none`, arguments: [objval] },
      };
...
function flattenExprToVal(
  e: AST.Expr<Type>,
  env: GlobalEnv
): [Array<IR.VarInit<Type>>, Array<IR.Stmt<Type>>, IR.Value<Type>] {
...
function pushStmtsToLastBlock(
  blocks: Array<IR.BasicBlock<Type>>,
  ...stmts: Array<IR.Stmt<Type>>
) {
  blocks[blocks.length - 1].stmts.push(...stmts);
}
