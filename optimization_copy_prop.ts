...
import { Env, copyEnv } from "./optimization"
...
//         return this.copyVars.get(arg).forward;
//     }
...
//             this.copyVars.set(copyTo, {tag: "copyId", forward: copyTo, reverse: []});            
//         }
//     }
...
export function optimizeValue(val: Value<any>, env: Env): Value<any>{
    ...
    if (env.has(val.name)){
        ...
        val = { name: env.get(val.name).forward, ...val };
    }
    return val;
}
...
// export function computeInitEnvForCopyProp(varDefs: Array<VarInit<any>>): Env{
...
export function computeInitEnvForCopyProp(varDefs: Array<VarInit<any>>, dummyEnv: boolean): Env{
    var env: Env = new copyEnv(new Map<string, copyVal>());
    varDefs.forEach(def => {
        if (!dummyEnv)
            env.set(def.name, {tag: "copyVal", value: {
                tag: "id",
                name: def.name
            }});
        else
